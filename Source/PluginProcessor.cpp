...
juce::AudioProcessorValueTreeState::ParameterLayout PitchBendGateAudioProcessor::createParameters()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterInt>("Gate positive value","Gate positive value",8193,16382,8193));
    layout.add(std::make_unique<juce::AudioParameterInt>("Gate negative value","Gate negative value",1,8191,8191));
    return layout;
}
